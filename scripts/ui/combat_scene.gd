...
var _bribe_demand_item: Dictionary = {}
...
func _on_talk(approach: String) -> void:
	match approach:
		"Reason":
			...
			var success: bool = randi() % 10 < (5 + score)
			if success:
				player.gold += enemy.gold_reward / 2
				_log("[color=lime]%s is persuaded and backs down.[/color]" % enemy.enemy_name)
				await get_tree().create_timer(1.5).timeout
				_end_combat("talk")
			else:
				_log("[color=red]%s isn't convinced![/color]" % enemy.enemy_name)
				var p_hp_before: int = player.hp
				var e_msg: String    = _apply_enemy_turn()
				_refresh_hp()
				if player.hp < p_hp_before:
...
func _start_bribe() -> void:
	_hide_actions()
	_right_back_btn.hide()
	_right_title.text = "BRIBE"
	_right_title.add_theme_color_override("font_color", Color(1.0, 0.75, 0.2))
	for child: Node in _right_list.get_children():
		child.queue_free()

	# Find a matching item in player inventory
	var wants: String = enemy.bribe_wants
	...
	for item: Dictionary in player.inventory:
		...
		match wants:
			...
			_:
				demand = item
				break
	_bribe_demand_item = demand

	var gold_cost: int = enemy.gold_reward
	if not demand.is_empty():
		_log("[color=orange]%s: \"Hand over your %s... or pay %d gold!\"[/color]" % [
				enemy.enemy_name, demand["name"], gold_cost])
		var give_btn: Button = Button.new()
		give_btn.text                = "Give %s" % demand["name"]
		give_btn.custom_minimum_size = Vector2(0, 28)
		give_btn.pressed.connect(func() -> void: await _resolve_bribe("give"))
		_right_list.add_child(give_btn)
	else:
		_log("[color=orange]%s: \"Pay me %d gold or face my wrath!\"[/color]" % [
				enemy.enemy_name, gold_cost])

	var gold_btn: Button = Button.new()
	gold_btn.text                = "Pay %d Gold" % gold_cost
	gold_btn.custom_minimum_size = Vector2(0, 28)
	gold_btn.disabled            = player.gold < gold_cost
	gold_btn.pressed.connect(func() -> void: await _resolve_bribe("gold"))
	_right_list.add_child(gold_btn)

	var refuse_btn: Button = Button.new()
	refuse_btn.text                = "Refuse"
	refuse_btn.custom_minimum_size = Vector2(0, 28)
	refuse_btn.pressed.connect(func() -> void: await _resolve_bribe("refuse"))
	_right_list.add_child(refuse_btn)


func _resolve_bribe(choice: String) -> void:
	_show_main_actions()
	_set_buttons(false)
	match choice:
		"give":
			player.remove_item(_bribe_demand_item, 1)
			_log("[color=lime]You hand over the %s.\n%s pockets it and lets you go.[/color]" % [
					_bribe_demand_item["name"], enemy.enemy_name])
			await get_tree().create_timer(1.5).timeout
			_end_combat("talk")
		"gold":
			player.gold -= enemy.gold_reward
			_log("[color=lime]You pay %d gold.\n%s counts it and backs away.[/color]" % [
					enemy.gold_reward, enemy.enemy_name])
			await get_tree().create_timer(1.5).timeout
			_end_combat("talk")
		"refuse":
			_log("[color=red]%s: \"Wrong answer!\"[/color]" % enemy.enemy_name)
			var p_hp_before: int = player.hp
			var e_msg: String    = _apply_enemy_turn()
			_refresh_hp()
			if player.hp < p_hp_before:
...
func _start_recruit() -> void:
	_talk_trust  = 0
	_talk_rounds = 2
	_hide_actions()
	_right_back_btn.hide()
	_show_recruit_submenu()


func _show_recruit_submenu() -> void:
	_right_title.text = "RECRUIT  %d/4" % _talk_trust
	_right_title.add_theme_color_override("font_color", Color(0.40, 1.0, 0.60))
	for child: Node in _right_list.get_children():
		child.queue_free()

	var round_lbl: Label = _dim_label("Round %d of 2" % (3 - _talk_rounds))
	_right_list.add_child(round_lbl)
	...
	for opt: Array in opts:
		...
		btn.text                = opt[1] as String
		btn.custom_minimum_size = Vector2(0, 28)
		btn.pressed.connect(func() -> void: await _resolve_recruit(key))
		_right_list.add_child(btn)


func _resolve_recruit(approach: String) -> void:
	var personality_match: bool = false
	match enemy.talk_personality:
	...
	var gain: int = randi() % 4 + (2 if personality_match else 0) - enemy.talk_difficulty
	gain         = max(0, gain)
	_talk_trust += gain
	_talk_rounds -= 1
	...
	var reaction: String
	if gain >= 3:
		reaction = _recruit_good_reaction()
	elif gain >= 1:
		reaction = _recruit_neutral_reaction()
	else:
		reaction = _recruit_bad_reaction()
	_log("[color=aqua]%s[/color]" % reaction)

	if _talk_trust >= 4:
		_show_main_actions()
		_set_buttons(false)
		if not (enemy.enemy_name in player.recruited):
			player.recruited.append(enemy.enemy_name)
		_log("[color=yellow]%s agrees to join you![/color]" % enemy.enemy_name)
		await get_tree().create_timer(1.5).timeout
		_end_combat("talk")
		return
	...
	if _talk_rounds <= 0:
		_show_main_actions()
		_set_buttons(false)
		_log("[color=red]%s: \"Enough talk!\"[/color]" % enemy.enemy_name)
		var p_hp_before: int = player.hp
		var e_msg: String    = _apply_enemy_turn()
		_refresh_hp()
		if player.hp < p_hp_before:
...
func _recruit_good_reaction() -> String:
	match enemy.talk_personality:
		"cowardly": return "%s: \"Y-you'd really protect me? ...maybe I'll consider it.\"" % enemy.enemy_name
		"proud":    return "%s: \"You recognize my power. I'll hear you out.\"" % enemy.enemy_name
		"greedy":   return "%s: \"Heh, finally someone smart. Keep talking.\"" % enemy.enemy_name
		"lonely":   return "%s: \"You really mean that? ...I'm listening.\"" % enemy.enemy_name
	return "%s seems interested." % enemy.enemy_name


func _recruit_neutral_reaction() -> String:
	match enemy.talk_personality:
		"cowardly": return "%s: \"Hmm... not convinced yet, but go on.\"" % enemy.enemy_name
		"proud":    return "%s: \"Interesting. You might be worth my time.\"" % enemy.enemy_name
		"greedy":   return "%s: \"You've got my attention. What else?\"" % enemy.enemy_name
		"lonely":   return "%s: \"...I suppose you're not so bad.\"" % enemy.enemy_name
	return "%s hesitates." % enemy.enemy_name


func _recruit_bad_reaction() -> String:
	match enemy.talk_personality:
		"cowardly": return "%s: \"That's not going to work on me!\"" % enemy.enemy_name
		"proud":    return "%s: \"Ha! Is that the best you've got?\"" % enemy.enemy_name
		"greedy":   return "%s: \"Words are cheap. Show me something real.\"" % enemy.enemy_name
		"lonely":   return "%s: \"...You don't really mean that.\"" % enemy.enemy_name
	return "%s is unmoved." % enemy.enemy_name


# ── Threaten ──────────────────────────────────────────────────────────────────

func _start_threaten() -> void:
	_talk_fear   = 0
	_talk_rounds = 2
	_hide_actions()
	_right_back_btn.hide()
	_show_threaten_submenu()


func _show_threaten_submenu() -> void:
	var threshold: int = 3 + enemy.talk_difficulty
	_right_title.text = "THREATEN  %d/%d" % [_talk_fear, threshold]
	_right_title.add_theme_color_override("font_color", Color(1.0, 0.40, 0.20))
	for child: Node in _right_list.get_children():
		child.queue_free()
	...
	for opt: Array in opts:
		...
		btn.text                = opt[1] as String
		btn.custom_minimum_size = Vector2(0, 28)
		btn.pressed.connect(func() -> void: await _resolve_threaten(key))
		_right_list.add_child(btn)


func _resolve_threaten(approach: String) -> void:
	var threshold: int  = 3 + enemy.talk_difficulty
	var gain: int       = 0
	...
	match approach:
		"Boast":
			gain = max(0, player.effective_str() - enemy.str + randi() % 3)
		"Intimidate":
			gain = max(0, player.effective_mag() - enemy.talk_difficulty + randi() % 3)
		"Bluff":
	...
	if insta_fail:
		_show_main_actions()
		_set_buttons(false)
		_log("[color=red]%s: \"You're bluffing! Have at you!\"[/color]" % enemy.enemy_name)
		var p_hp_before: int = player.hp
		var e_msg: String    = _apply_enemy_turn() + "\n" + _apply_enemy_turn()
		_refresh_hp()
		if player.hp < p_hp_before:
	...
	if _talk_fear >= threshold:
		_show_main_actions()
		_set_buttons(false)
		_log("[color=orange]%s[/color]" % _threaten_scared_reaction())
		_log("[color=lime]%s backs down![/color]" % enemy.enemy_name)
		await get_tree().create_timer(1.5).timeout
		_end_combat("talk")
		return

	_log("[color=orange]%s[/color]" % _threaten_defiant_reaction(gain))

	if _talk_rounds <= 0:
		_show_main_actions()
		_set_buttons(false)
		_log("[color=red]%s decides to fight![/color]" % enemy.enemy_name)
		var p_hp_before: int = player.hp
		var e_msg: String    = _apply_enemy_turn()
		_refresh_hp()
		if player.hp < p_hp_before:
...
func _threaten_scared_reaction() -> String:
	match enemy.talk_personality:
		"cowardly": return "%s: \"OK OK! I'll go! Don't hurt me!\"" % enemy.enemy_name
		"proud":    return "%s: \"...I'll let you go this time.\"" % enemy.enemy_name
		"greedy":   return "%s: \"Fine! Not worth dying over this!\"" % enemy.enemy_name
		"lonely":   return "%s: \"...I just wanted company. I'm leaving.\"" % enemy.enemy_name
	return "%s: \"I'll go.\"" % enemy.enemy_name


func _threaten_defiant_reaction(gain: int) -> String:
	if gain <= 0:
		match enemy.talk_personality:
			"cowardly": return "%s: \"Y-you don't scare me that much!\"" % enemy.enemy_name
			"proud":    return "%s: \"Ha! Is that a threat?\"" % enemy.enemy_name
			"greedy":   return "%s: \"My life's worth more than you think!\"" % enemy.enemy_name
			"lonely":   return "%s: \"...Why won't you just leave me alone?\"" % enemy.enemy_name
	else:
		match enemy.talk_personality:
			"cowardly": return "%s: \"W-wait... maybe we can work this out...\"" % enemy.enemy_name
			"proud":    return "%s: \"You're... stronger than you look.\"" % enemy.enemy_name
			"greedy":   return "%s: \"Alright, maybe this isn't worth the risk...\"" % enemy.enemy_name
			"lonely":   return "%s: \"Stop... please...\"" % enemy.enemy_name
	return "%s hesitates." % enemy.enemy_name
